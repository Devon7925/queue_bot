...
use poise::{
    serenity_prelude::{
        self as serenity, Builder, ChannelId, ChannelType, CreateButton, CreateChannel, CreateMessage, EditMember, EditMessage, GuildId, Mentionable, UserId
    },
    CreateReply,
};
use rand::Rng;
            ...
            if let Some(message_component) = interaction.as_message_component() {
                ...
                if matches!(vote_type, VoteType::Map) {
                    ...
                    let mut content = {
                        ...
                        for (vote_type, count) in votes {
                            content += format!("{}: {}", vote_type, count).as_str();
                ...
                if matches!(vote_type, VoteType::Result) {
                    ...
                    let content = {
                        ...
                        for (_user, vote) in match_data.get(&match_number).unwrap().result_votes.iter()
                        {
                            let current_votes = votes.get(&vote).unwrap_or(&0);
                            votes.insert(vote.clone(), current_votes + 1);
                        }
...
async fn try_matchmaking(
    ...
    for i in 0..team_count {
        vc_channels.push(CreateChannel::new(format!("Team {} - #{}", i+1, new_idx))
            .category(category.clone())
    ...
    if config.map_vote_count > 0 {
        ...
        for _ in 0..config.map_vote_count {
            ...
            map_vote_message = map_vote_message.button(CreateButton::new(format!("map_{}", rand_map).clone()).label(rand_map).style(serenity::ButtonStyle::Secondary));
        }
        match_channel
            .send_message(
                cache_http.clone(),
...
async fn configure_queue_category(
    ctx: Context<'_>,
    #[description = "Queue category"] #[channel_types("Category")] new_value: Option<serenity::Channel>,
) -> Result<(), Error> {
    if let Some(new_value) = new_value {
...
async fn configure_queue_channels(
    ctx: Context<'_>,
    #[description = "New queue channel"] #[channel_types("Voice")] new_value: Option<serenity::Channel>,
) -> Result<(), Error> {
    if let Some(new_value) = new_value {
        let response = {
            let mut data_lock = ctx.data().configuration.lock().unwrap();
...
async fn configure_post_match_channel(
    ctx: Context<'_>,
    #[description = "Post match channel"] #[channel_types("Voice")] new_value: Option<serenity::Channel>,
) -> Result<(), Error> {
    if let Some(new_value) = new_value {
        let response = {
            let mut data_lock = ctx.data().configuration.lock().unwrap();
...
async fn main() {
    ...
    let framework = poise::Framework::builder()
        .options(poise::FrameworkOptions {
            ...
            commands: vec![register(), configure(), export_config(), list_queued()],
            ..Default::default()
        })
